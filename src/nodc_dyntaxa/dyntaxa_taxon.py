...
class DyntaxaTaxon:
    ...
    def __init__(self, path: str | pathlib.Path, filter_list: list[str] | None = None):
        ...
        self._path = pathlib.Path(
            self._original_path.parent,
            f"{self._original_path.stem}_fixed{self._original_path.suffix}",
        )
    ...
    def _load_file(self) -> None:
        self._df = pl.read_csv(
            ...
            ignore_errors=True,
            schema_overrides={"taxonRemarks": str},
        )

    def _cleanup_data(self) -> None:
        self._df = self._df.filter(~pl.col(self.first_col).str.starts_with("#"))
        self._df = self._df.filter(~pl.col(self.first_col).str.contains("TaxonName"))
        # self._df = self._df.filter(pl.col('taxonomicStatus').eq('accepted'))
        self._df = self._df.with_columns(
            pl.col("taxonId")
            .map_elements(lambda x: x.split(":")[-1].strip(), return_dtype=str)
            .alias("taxon_id")
        )

        self._df = self._df.with_columns(
            pl.col("taxonId")
            .map_elements(lambda x: x.split(":")[-2].strip(), return_dtype=str)
            .alias("taxon_id_tag")
        )
        ...
        self._df = self._df.with_columns(
            pl.col("parentNameUsageID")
            .map_elements(lambda x: x.split(":")[-1].strip(), return_dtype=str)
            .alias("parent_taxon_id")
        )

        if self._filter_list:
            self._df.filter(
                pl.col("scientificName").str.to_lowercase().is_in(self._filter_list)
            )
    ...
    def get_name_list(self) -> list[str]:
        return list(
            self._df.filter(pl.col("taxonomicStatus") == "accepted")["scientificName"]
        )
